...
export class AiqfomeAuthService {
  ...
  buildOAuthUrl(storeId?: string) { const clientId = this.configService.get<string>('AIQFOME_CLIENT_ID'); const redirectUri = this.configService.get<string>('AIQFOME_REDIRECT_URI'); const scope = this.configService.get<string>('AIQFOME_SCOPE') || 'orders logistic'; return `https://id.magalu.com/oauth/authorize?response_type=code&client_id=${encodeURIComponent(clientId || '')}&redirect_uri=${encodeURIComponent(redirectUri || '')}&scope=${encodeURIComponent(scope)}&state=${encodeURIComponent(storeId || '')}`; }
  async handleCallback(code: string, storeId: string) { if (!code || !storeId) throw new BadRequestException('code e storeId são obrigatórios'); const tokenData = await this.exchangeCodeForToken(code); await this.saveTokens(storeId, tokenData); return { success: true, storeId }; }
